...
lbu_cache_dir = os.environ.get(
    "LBU_CACHE_DIR",
    os.path.expanduser("~/.cache/lbu") if os.getuid() else "/var/cache/lbu",
)
...
class FilesystemError(LookupError):
    pass


class NotAufs(ValueError):
    pass


class NotLoopDev(ValueError):
    pass


class NotSFS(ValueError):
    pass
...
def clear_cached_properties(obj):
    for prop in [
        p
        for p in [getattr(obj.__class__, n) for n in dir(obj.__class__)]
        if isinstance(p, property) and hasattr(obj, "__cached__" + p.fget.__name__)
    ]:
...
def cli_func(func=None, name=None, parse_argv=None, desc=None):
    if func is None:
        ...
        def gen(func_real):
            if name is not None:
                func_real._cli_name = name
            if parse_argv is not None:
                func_real._cli_parse_argv = parse_argv
            if desc is not None:
                func_real._cli_desc = desc
            return cli_func(func_real)

        return gen
    cli_func.commands[getattr(func, "_cli_name", func.__name__.replace("_", "-"))] = (
        func
    )
    ...
    def cli_call(argv):
        try:
            args, kwargs = func._cli_parse_argv(argv)
        except Exception as e:
    ...
    if not func.__doc__:
        ...
        defaults = (
            dict(
                [
                    (rev_args[i_d[0]], i_d[1])
                    for i_d in enumerate(reversed(spec.defaults))
                ]
            )
            if spec.defaults
            else {}
        ...
        func.__doc__ = " ".join(
            [
                "[<%s>=%r]" % (n, defaults[n]) if n in defaults else "<%s>" % n
                for n in spec.args
            ]
...
class CLIProgressReporter(object):
    ...
    def __init__(self, full_size, **attrs):
        ...
        self.report_buckets = [
            i * full_size / self.nr_buckets for i in range(self.nr_buckets)
        ]
        list(map(lambda k: setattr(self, k, attrs[k]), attrs))

    def __call__(self, sz):
        if sz is None:
            print("100%", file=self.output_stream)
        elif self.report_buckets and sz >= self.report_buckets[0]:
            print(
                "%d%%.." % (100 * sz / self.full_size), end=" ", file=self.output_stream
            )
...
"""
        ...
        def apparmor_cfg(self):
            return (
                "lxc.apparmor.profile = %s" % (self.apparmor_profile)
        ...
        def dev_cfg(self):
            ...
            return "\n".join(
                ...
                + ["lxc.cgroup.devices.allow = %s" % d for d in self.devices_allow]
            )
        ...
        def cap_cfg(self):
            if self.cap_keep:
                return "lxc.cap.keep = %s" % (
                    " ".join(
                        self.cap_keep,
                    )
                )
            elif self.cap_drop:
                return "lxc.cap.drop = %s" % (
                    " ".join(
                        self.cap_drop,
                    )
                )
...
"""
            ...
            def link_cfg(self):
                return (
                    (("lxc.net.%(netnum)d.link = %(link)s") % self) if self.link else ""
                )

            @cached_property
            def ip_cfg(self):
                return (
                    (("lxc.net.%(netnum)d.ipv4.address = %(ip)s") % self)
                    if self.ip
                    else ""
                )
            ...
            def gw_cfg(self):
                return (
                    (("lxc.net.%(netnum)d.ipv4.gateway = %(gw)s") % self)
                    if self.gw
                    else ""
                )
...
%(gw_cfg)s"""
    ...
    def from_sfs_ext(cls, name, sfs_parts, extra_parts=[], bind_dirs=[], **attrs):
        cmd = [
            ...
            "--",
            "--default-parts",
            " ".join(map(str, sfs_parts)),
            "--host-network",
        ]
        cmd.extend(
            reduce(
                lambda a, b: a + ["--bind-ro" if b.ro else "--bind", str(b)],
                bind_dirs,
                [],
            )
        )
    ...
    @classmethod
    def from_sfs(
        cls, name, sfs_parts, bind_dirs=None, veth=None, vlan=None, nonet=False, **attrs
    ):
            ...
            return run_command(
                ...
                env=dict(
                    [
                        (k, os.environ[k])
                        for k in [
                            k for k in list(os.environ.keys()) if k.startswith("LXC_")
                        ]
                    ]
                ),
            )
    ...
    def apt_install(self, packages):
        self.run(
            [
                ...
                "-x",
                "-c",
                'dpkg -s "$@" >/dev/null || (apt-get update || apt-get update && apt-get install -y "$@" )',
                "_",
            ]
            + packages,
            show_output=True,
...
class SFSFinder(object):
    ...
    def _sfs_dirs(self):
        if "SFS_FIND_PATH" in os.environ:
            ...
            dirlist = [
                FSPath(MountPoint(e["mnt"]).loop_backend).parent_directory.path
                for e in [e for e in global_mountinfo if e["fs_type"] == "squashfs"]
            ]
        return dict(
            [
                (p, SFSDirectory(p))
                for p in [d for d in uniq_list(dirlist) if os.path.exists(d)]
            ]
        )
...
class SFSBuilder(object):
    ...
    default_lxc_parts = os.environ.get(
        "LXC_BUILD_SFS_PARTS", "00-* scripts settings"
    ).split()

    USRMERGE_DIRS = os.environ.get(
        "LXC_USRMERGE_DIRS", "lib bin sbin lib32 lib64 libx32"
    ).split()
    ...
    def __init__(self, target_sfs, source=None):
        ...
        if source is None and target_sfs.exists:
            ...
            try:
                self.run_env.update(
                    _load_build_env(
                        target_sfs.open_file(self.BUILD_ENV_PATH, "r").read()
                    )
                )
            except IOError:
                pass
    ...
    def dest_base(self):
        dest_base = FSPath(
            os.path.join(self.dest_dir_parent, self.name), auto_remove=True
        )
    ...
    def dest_dir(self):
        ...
        if self.source is not None:
            ...
            git_tar_out = (
                'cd "$SRC";git archive HEAD | tar x -C "$DESTDIR";'
    ...
    def lxc_rw_d(self):
        if os.environ.get("LXC_RW_D"):
            return FSPath(os.environ["LXC_RW_D"])
    ...
    def lxc_parts(self):
        ...
        try:
            ret = (
                self.dest_dir.open_file(self.LXC_PARTS_FILE, "r").read().strip().split()
            )
        except IOError as e:
    ...
    def deb_mappings(self):
        ...
        ret = [
            (cache_dir.join("archives"), "var/cache/apt/archives"),
    ...
    def lxc(self):
        lxc = LXC.from_sfs(
            self.name,
            self.lxc_parts + [d.path for d in [self.lxc_setup_d, self.lxc_rw_d]],
            self.bind_dirs,
    ...
    def build_shell(self):
        try:
            run_command(
                ["bash", "--rcfile", self.SFS_BUILD_PROFILE_SH, "-i"],
                cwd=self.dest_dir.path,
                show_output=True,
                env=dict(
                    self.run_env,
                    sfs_build_target=self.target.basename,
                    DESTDIR=self.dest_dir.path,
                ),
            )
        except CommandFailed as e:
    ...
    def build(self):
        ...
        if "BUILD_SCRIPT" in self.run_env:
            self.run_in_dest(
                ["sh", "-c", self.run_env["BUILD_SCRIPT"]], show_output=True
            )
        ...
        for script in sorted(
            self.sfs_src_d.walk(pattern="[0-9][0-9]-*"), key=lambda p: p.basename
        ):
            if not apt_updated:
                self.run_in_dest(["apt-get", "update"], show_output=True)
                apt_updated = True
            before_build_script = os.environ.get(
                "BEFORE_BUILD_{0}".format(
                    script.basename.replace("-", "_").replace(".", "_")
                )
            )
            ...
            cmd = [
                os.path.join(
                    self.LXC_DESTDIR, self.SFS_SRC_D.lstrip("/"), script.basename
                )
            ]
            try:
        ...
        if self.source is not None:
            ...
            if git_source_url is not None:
                ...
                self.dest_dir.open_file(self.GIT_COMMIT_PATH, "w").write(
                    self.source.last_commit
                )
...
class SFSDirectory(object):
    ...
    def __init__(self, backend):
        if isinstance(backend, str):
            if (
                ...
                or backend.startswith("http://")
                or backend.startswith("https://")
            ):
                backend = FSPath(backend, walk_pattern="*.sfs")
            elif os.path.isdir(os.path.dirname(backend)):
                backend = FSPath(
                    os.path.dirname(backend),
                    walk_pattern=os.path.basename(backend),
                    walk_depth=0,
                )
        ...
        if isinstance(backend, FSPath):
            ...
            raise ValueError(
                "Unknown backend: (%s) %r" % (type(backend).__name__, backend)
            )
    ...
    def all_sfs(self):
        return list(
            sorted(
                self.backend.walk(file_class=SFSFile, depth=self.sfs_search_depth),
                key=lambda s: s.basename,
            )
        )
            ...
            if old_sfs.exists:
                for cur_sfs in cur_sfs_lst:
                    if os.path.samefile(cur_sfs.path, old_sfs.path):
                        continue
            try:
                link_target = old_sfs.symlink_target
            except OSError:
                pass
            else:
                if "/" not in link_target:
                    ...
                    if old_tgt.exists:
                        for cur_sfs in old_sfs.parent_directory.walk(
                            pattern="*.sfs", depth=0
                        ):
...
class FSPath(object):
    ...
    def __new__(cls, path, **attrs):
        ...
        if cls == FSPath and (
            path_str.endswith(".sfs") or path_str.endswith(".sfs.OLD")
        ):
            cls = SFSFile
    ...
    def __init__(self, path, **attrs):
        ...
        if not isinstance(path, str):
            raise ValueError(
                "Invalid init path type for %s: %s"
                % (self.__class__.__name__, type(path).__name__)
            )
        self.path = path
    ...
    def backend(self):
        if self.mountpoint.fs_type == "aufs":
            for mpt in [
                c.mountpoint for c in reversed(self.mountpoint.aufs_components)
            ]:
                if self.path in mpt:
                    return FSPath(mpt.loop_backend)
    ...
    def makedirs(self, mode=0o755, sudo=False):
        if not self.exists:
            if sudo:
                run_command(
                    ["mkdir", "-m", "%04o" % (mode), "-p", self.path], as_user="root"
                )
            else:
    ...
    def walk(self, pattern=None, file_class=None, exclude=None, depth=None):
        ...
        for d, dn, fn in self._walk_func(self.path):
            ...
            for f in [
                n for n in fn if any([fnmatch.fnmatch(n, pat) for pat in pattern])
            ]:
    ...
    def file_info(self):
        ...
        try:
            ...
            try:
                ...
                ret["mtime"] = datetime.datetime.fromtimestamp(
                    st.st_mtime, UTC()
                ).isoformat()
    ...
    def file_tree(self):
        ...
        for f in self.walk():
            ...
            dir_entry = reduce(
                lambda a, b: a.setdefault("dirs", {}).setdefault(b, {}), path_parts, ret
            )
    ...
    def mountpoint(self):
        ...
        path_components = (self.path if self.islink() else self.realpath().path).split(
            os.path.sep
        )
        sub_paths = [
            FSPath(os.path.sep.join(path_components[: n + 1]) or os.path.sep)
            for n in range(len(path_components))
        ]
        cur_path = self.path
    ...
    def _parent_path(self):
        parent_path = os.path.sep.join(
            os.path.realpath(self.path).rsplit(os.path.sep, 1)[:-1]
        )
        if not parent_path:
    ...
    def loop_dev(self):
        try:
            alt_path = self.aufs_original.path
        except NotAufs:
            alt_path = self.path
        for devname in os.listdir("/sys/block"):
            ...
            try:
                bfile = (
                    open("/sys/block/%s/loop/backing_file" % (devname,)).read().strip()
                )
            except IOError:
                continue
            if not os.path.exists(bfile):
                continue
            if not os.path.samefile(bfile, self.path) and not os.path.samefile(
                bfile, alt_path
            ):
                continue
            if (
                not int(open("/sys/block/%s/loop/offset" % (devname,)).read().strip())
                == 0
            ):
                continue
...
class MountInfo(object):
    ...
    def proc_mountinfo_line(line):
        ...
        ret = dict(
            ...
            st_dev=reduce(
                lambda a, b: (a << 8) + b, list(map(int, parts[2].split(":")))
            ),
            root=MountInfo.decode_escapes(parts[3]),
            mnt=MountInfo.decode_escapes(parts[4]),
            opts_mnt=set(parts[5].split(",")),
            opt_fields=set(),
        )
        idx = 6
        while not parts[idx] == "-":
...
class GitRepo(FSPath):
    ...
    def source_url(self):
        try:
            remote, branch = run_command(
                ["git", "rev-parse", "--abbrev-ref", "@{upstream}"], cwd=self.path
            ).split("/")
        except CommandFailed:
            return None
        return "%s#%s" % (
            run_command(
                ["git", "config", "--get", "remote.%s.url" % (remote,)], cwd=self.path
            ),
            branch,
        )
...
class Downloader(object):
    ...
    _env_proxy_vars = " ".join(
        [n for n in list(os.environ.keys()) if n.lower().endswith("_proxy")]
    )
    ...
    def proxy_env(self):
        return dict(
            [
                (n, os.environ[n])
                for n in [n for n in self.pass_host_env if n in os.environ]
            ]
        )
            ...
            if dest_st.st_size > 0:
                opener.addheaders.append(
                    (
                        "If-Modified-Since",
                        time.strftime(
                            self.http_time_format, time.gmtime(dest_st.st_mtime)
                        ),
                    )
                )
        try:
    ...
    def dl_file(self, source, fname=None, dest_dir=None):
        ...
        if fname is None:
            fname = "%s-%s" % (
                hashlib.md5(source.encode("utf-8")).hexdigest()[:8],
...
class SFSFile(FSPath):
    ...
    class SFSBasename(str):
        ...
        def __eq__(self, other):
            if super(SFSFile.SFSBasename, self) == other:
                return True
            ...
            if self.strip_down() == other.strip_down():
                return True
            try:
                without_sfs = self[: self.rindex(".sfs")]
            except ValueError:
                without_sfs = self
    ...
    def sfs_directory(self):
        if not self.validate_sfs():
            raise NotSFS("Not a SFS file", self.path)
        return SFSDirectory(self._parent_path)

    @cached_property
    def basename(self):
        return self.SFSBasename(super(SFSFile, self).basename)
    ...
    def git_source(self):
        try:
            git_source = self.open_file(self.GIT_SOURCE_PATH, "r").read().strip()
        except IOError:
            return
        if "#" in git_source:
    ...
    def git_commit(self):
        try:
            return self.open_file(self.GIT_COMMIT_PATH, "r").read().strip()
        except IOError:
            pass

    @cached_property
    def git_branch(self):
        if self.git_source is None:
            return
        try:
    ...
    def git_repo(self):
        return dl.dl_file(
            ...
            else "%s#%s" % (self.git_source, self.git_branch)
        )
...
class MountPoint(FSPath):
    ...
    @cached_property
    def fs_type(self):
        return self.mountinfo["fs_type"]

    @cached_property
    def mount_source(self):
        return self.mountinfo["dev"]

    @cached_property
    def mount_options(self):
        return self.mountinfo["opts"]
    ...
    def _mount_exec(self, src, *opts, **kwargs):
        ...
        if opts or kwargs:
            cmd.extend(
                [
                    "-o",
                    ",".join(list(opts) + ["%s=%s" % (k, kwargs[k]) for k in kwargs]),
                ]
            )
        run_command(cmd, as_user="root")
    ...
    @cached_property
    def aufs_si(self):
        return list(filter(lambda x: x.startswith("si="), self.mount_options))[0].split(
            "="
        )[1]

    @cached_property
    def aufs_components(self):
        if not self.fs_type == "aufs":
            raise NotAufs("Mountpoint is not aufs", self.path)
        components = []
        glob_prefix = "/sys/fs/aufs/si_%s/br" % (self.aufs_si,)
        for branch_file in sorted(
            glob.glob(glob_prefix + "[0-9]*"), key=lambda v: int(v[len(glob_prefix) :])
        ):
            branch_dir, branch_mode = open(branch_file).read().strip().rsplit("=", 1)
            components.append(
                FSPath(
                    branch_dir,
                    aufs_mode=branch_mode,
                    aufs_index=int(branch_file[len(glob_prefix) :]),
                )
            )
        return components
    ...
    def fs_components(self):
        if self.fs_type == "aufs":
            return self.aufs_components
        elif self.fs_type == "overlay":
            return self.overlay_components
        else:
            raise NotAufs("Need aufs or overlay mountpoint")
    ...
    def loop_backend(self):
        ...
        if not loop_name.startswith("loop"):
            raise NotLoopDev("Mountpoint does not seem to be loop device", loop_name)
        return (
            open(os.path.join("/sys/block", loop_name, "loop/backing_file"))
            .read()
            .rstrip("\n")
        )
    ...
    def mount_combined(self, parts, **kwargs):
        ...
        if os.environ.get("LXC_RW_D"):
            ...
            try:
                dirs.remove(rw_mount.path)
            except ValueError:
                pass
        else:
            rw_mount = MountPoint(
                FSPath(lbu_cache_dir).join(
                    "comnt-rw-%s-%s" % (os.getpid(), time.time())
                )
            )
        ...
        if kwargs["fs_type"] == "aufs":
            dirs_arg = ":".join(
                ["%s=rw" % (rw_mount.path,)] + ["%s=ro" % (d,) for d in reversed(dirs)]
            )
            ...
            raise NotImplementedError(
                "combined fs_type=%r is not implemented" % (kwargs["fs_type"])
            )
...
class KVer(object):
    def __init__(self, s):
        self.value = [
            [int(y) if y.isdigit() else y for y in z.split(".")] for z in s.split("-")
        ]
...
class SourceList(FSPath):
    base_url = None
    kernel_fn_re = re.compile(
        r".*?(?P<arch>x86_64|i[3-6]86)/(?:[0-9][0-9]-)?kernel-(?P<kver>[0-9]+\.[0-9].*)\.sfs$"
    )
...
class BootDirBuilder(FSPath):
    dist_dirname = "sfs"
    build_targets = set(
        os.environ.get("LBU_BUILD_TARGETS", "efi sfs ramdisk grubconf vmlinuz").split()
    )
    mkrd_src_url = os.environ.get(
        "LBU_MKRD_URL", "https://github.com/SFSLiveBoot/make-ramdisk.git"
    )
    ...
    mkrd_pkgs = os.environ.get(
        "LBU_MKRD_PKGS",
        "make binutils udev klibc-utils lvm2 kmod cryptsetup-run busybox pciutils fakeroot cpio",
    ).split()
    grub_pkgs = os.environ.get(
        "LBU_GRUB_PKGS", "grub2-common grub-efi-amd64-bin grub-pc-bin mtools xorriso"
    ).split()
    ...
    def lxc_buildconf_d(self):
        d = MountPoint(
            os.path.join(lbu_cache_dir, "builder-lxc-conf-%d" % (os.getpid(),)),
            auto_remove=True,
        )
        if not d.is_mounted:
    ...
    def build_lxc(self):
        ...
        if (
            "vmlinuz" in self.build_targets
            or "ramdisk" in self.build_targets
            or "ramdisk_net" in self.build_targets
            or "sfs" in self.build_targets
        ):
        ...
        if self.iso_output:
            ...
            bind_dirs.append(
                LXC.BindEntry(
                    FSPath(self.iso_output).parent_directory.realpath(),
                    self.LXC_DEST_ISO_PARENT,
                )
            )
        lxc = LXC.from_sfs(
            "builddir-%d" % (os.getpid(),),
            auto_remove=True,
            sfs_parts=sfs_parts,
            bind_dirs=bind_dirs,
        )
    ...
    def build(self):
        ...
        if self.iso_output:
            ...
            lxc_iso = (
                FSPath(self.LXC_DEST_ISO_PARENT)
                .join(FSPath(self.iso_output).basename)
                .path
            )
            if self.grub_pkgs:
                self.build_lxc.apt_install(self.grub_pkgs)
            self.build_lxc.run(
                ["grub-mkrescue", "-o", lxc_iso, self.LXC_DEST_BOOTDIR],
                show_output=True,
            )
    ...
    def build_sfs(self):
        ...
        for src_url, sfs_name in source_list:
            ...
            if os.path.isfile(src_url) or (
                src_url.endswith(".sfs")
                and (src_url.startswith("http://") or src_url.startswith("https://"))
            ):
            ...
            if dest_sfs.exists and not dest_sfs.needs_update:
                info(
                    "No change: %s is up to date (%s)",
                    sfs_name,
                    stamp2txt(dest_sfs.create_stamp),
                )
                sfs_finder.register_sfs(dest_sfs)
                continue
    ...
    def build_vmlinuz(self):
        ...
        with self.arch_dir.open_file("vmlinuz-%s" % (self.kver), "wb") as vmlnz:
            vmlnz.write(
                self.kernel_sfs.open_file("boot/vmlinuz-%s" % (self.kver)).read()
            )
    ...
    def build_efi(self):
        ...
        self.build_lxc.run(
            ["grub-mkimage", "-o", lxc_efi_img, "-O", self.efi_arch, "-p", grub_prefix]
            + self.efi_mods
        )
...
def sfs_info(filename):
    ...
    ret = dict(
        basename=dict(stripped=sfs.basename.strip_down(), priority=sfs.basename.prio()),
...
def get_root_sfs():
    ...
    if not root_backend.validate_sfs():
        return RuntimeError("Not running under SFS-based system?")
    return root_backend


def run_as_root(*args):
    if os.getuid():
        os.execvp("sudo", ["sudo"] + list(args))
...
def run_command(cmd, cwd=None, show_output=False, env={}, as_user=None):
    ...
    proc = subprocess.Popen(
        cmd, env=cmd_env, cwd=cwd, stderr=subprocess.PIPE, stdout=subprocess.PIPE
    )
    stdout_buf = []
    stderr_buf = []
    ...
    while check_f:
        ...
        if not in_f:
            continue
        for buf, f, log_tag, sys_f in (
            (stdout_buf, proc.stdout, "stdout", sys.stdout),
...
def mount(src, dst, *opts, **kwargs):
    ...
    if opts or kwargs:
        cmd.extend(
            ["-o", ",".join(list(opts) + ["%s=%s" % (k, kwargs[k]) for k in kwargs])]
        )
    run_command(cmd, as_user="root")


@cli_func(
    parse_argv=lambda argv: (
        (argv[0],),
        {"pos": int(argv[1])} if len(argv) > 1 else {},
    ),
...
def _sfs_list_rm_empty(node):
    if "files" in node and not node["files"]:
        del node["files"]
    if "dirs" in node:
...
def gen_sfs_list(
    target_dir, exclude_pat="", include_pat="*.sfs,*/vmlinuz-*,*/ramdisk*"
):
    return FSPath(
        target_dir, walk_pattern=include_pat, walk_exclude=exclude_pat
    ).file_tree
...
@cli_func(
    parse_argv=_update_sfs_parse_args,
        ...
        for sfs in target_dir_all_sfs:
            ...
            try:
                if "/" in sfs.symlink_target:
                    info(
                        "Skipping non-local symlink: %s -> %s",
                        sfs.basename,
                        sfs.symlink_target,
                    )
                    continue
            ...
            if sfs.basename.strip_down() in skip_sfs:
                info(
                    "Skipping ('%s' listed in $SFS_UPDATE_SKIP)",
                    sfs.basename.strip_down(),
                )
                continue
            if source_dir == "--list":
                print(sfs.path)
            ...
            if cksum_file == "":
                ...
                cksum_file = ChecksumFile(cksum_file)
            if cksum_file:
            ...
            if source_dir == "--auto-rebuild":
                ...
                if dst_sfs.latest_stamp > dst_sfs.create_stamp:
                    ...
                    info(
                        "Keeping %s: latest %s %s current: %s",
                        dst_sfs.basename,
                        stamp2txt(dst_sfs.latest_stamp),
...
def build_boot_dir(path, source_list, dist_name="sfs", iso_output=None):
    builder = BootDirBuilder(
        path, source_list_url=source_list, dist_dirname=dist_name, iso_output=iso_output
    )
    builder.build()
...
def build_ramdisk(dest_dir, kver, arch, kernel_sfs, *makerd_args):
    ...
    for opt in makerd_args:
        ...
        makeargs[k] = v
    builder = BootDirBuilder(
        dest_dir, arch=arch, kver=kver, kernel_sfs=SFSFile(kernel_sfs), run_env={}
    )
...
def aufs_update_branch(mnt, aufs="/"):
    ...
    if cur_comp_match:
        ...
        aufs_mnt.mount(
            "aufs", "remount", "ins:%d:%s=rr" % (comp_match[0].aufs_index, cur_mnt)
        )
...
@cli_func(
    desc="Run LXC instance. bind is space-separated entries of <src_dir>=<dst_dir>[:ro]"
)
def lxc_run(
    name,
    init="exec bash -i >&0 2>&0",
    sfs_parts="00-* settings scripts",
    bind=None,
    vlan=None,
    veth=None,
    devs=None,
...
def build_efi(path, arch=os.uname()[4], kver=os.uname()[2]):
    builder = BootDirBuilder(
        path, build_targets={"efi", "grubconf"}, arch=arch, kver=kver, extra_dirs=[]
    )
    builder.build()
